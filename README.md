...
- **edit/write 게이트** — advisor를 한 번도 호출하지 않았다면 각 프롬프트의 첫 `edit` / `write` 호출을 한 번씩 막아 advisor 부터 부르게 한다(미호출 시 프롬프트마다 재발동).
- **연속 실패 게이트** — tool 실패를 연속으로 센다. 성공한 tool result(또는 advisor 호출, 새 프롬프트, 새 세션)가 카운터를 0 으로 리셋한다. 2회 연속 실패에서 경고를 붙이고 다음 툴 호출을 막는다(프롬프트당 1회, 성공 리셋 후 새 연속에서 재발동).
